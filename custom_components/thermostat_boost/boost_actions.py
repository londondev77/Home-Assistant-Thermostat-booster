...
async def _load_temperature_snapshot_store(hass: HomeAssistant) -> tuple[Store, dict[str, float]]:
...
def _get_snapshot_restore_pending(hass: HomeAssistant) -> set[str]:
    ...
    pending = domain_data.get(_SNAPSHOT_RESTORE_PENDING_KEY)
    if isinstance(pending, set):
        return pending
    if isinstance(pending, dict):
        migrated = set(pending)
        domain_data[_SNAPSHOT_RESTORE_PENDING_KEY] = migrated
        return migrated
    migrated = set()
    domain_data[_SNAPSHOT_RESTORE_PENDING_KEY] = migrated
    return migrated
...
def async_cancel_pending_scheduler_callbacks(hass: HomeAssistant, entry_id: str) -> None:
    ...
    if restore_cancelled:
        _LOGGER.debug(
            "Cancelled pending scheduler restore callback for %s",
            entry_id,
        )
...
def _schedule_snapshot_restore_retry(
    ...
    _LOGGER.debug(
        "Queueing scheduler restore retry for %s in %ss",
        entry_id,
...
async def async_finish_boost_for_entry(
    ...
    try:
        ...
        _LOGGER.debug(
            ...
            "(completion may be deferred/retried; timer_expired_while_offline=%s)",
            entry_id,
            expired_while_offline,
        )
    finally:
